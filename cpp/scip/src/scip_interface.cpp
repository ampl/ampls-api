...
std::vector<double> SCIPModel::getConstraintsValueImpl(int offset, int length) {
  ...
  SCIPwriteOrigProblem(getSCIPmodel(), "d:/original", "lp", 0);
  SCIPwriteTransProblem(getSCIPmodel(), "d:/trans", "lp", 0);
  if (SCIPisDualSolAvailable(getSCIPmodel(), TRUE)) {
    for (int i = offset; i < offset + length; i++)
    {
      SCIPgetDualSolVal(getSCIPmodel(),
        modelData()->linconss[i], pi.data() + i, NULL);
    }
  }
  return pi;
}
std::vector<double> SCIPModel::getVarsValueImpl(int offset, int length) {
  ...
  for (int i = offset; i < offset+length; i++)
    vars[i-offset] = SCIPgetSolVal(getSCIPmodel(), SCIPgetBestSol(getSCIPmodel()), SCIPgetProbData(getSCIPmodel())->vars[i]);
  return vars;
}
