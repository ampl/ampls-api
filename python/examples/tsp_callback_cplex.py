...
class MyCallback(ampls.CPLEXCallback):
    CALL_COUNT_MIPSOL = 0
    ...
    def mipsol(self):
        ...
        if len(groups) == 1:
            ...
            for grp in groups:
                ...
                # Note the last parameter defines if the cut is thread-local or global
                # Also note that the callback must derive from CPLEXCallback, not from 
                # GenericCallback
                self.add_lazy(cutvarnames, coeffs,
                             ampls.CutDirection.LE, len(grp)-1,0)
        return 0
    ...
    def mipnode(self):
        ...
        for i in range(1, len(vertices)):
            ...
            if max_flow <= 1-1e-5:
                ...
                coeffs = [1 for i in range(len(cutvarnames))]
                # Note the last parameter defines if the cut is thread-local or global
                self.add_cut(cutvarnames, coeffs, ampls.CutDirection.GE, 1, 0)
