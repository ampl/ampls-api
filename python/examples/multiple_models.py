"""A script that shows how to create multiple models with AMPL, export an AMPLs instance
...
def solveModel(ampl:amplpy.AMPL, options:Dict[str,Any] = {}, useNativeCall: bool=True):
    ...
    if useNativeCall:
        amplpy_gurobi.GRBoptimize(model.get_grb_model())
    else:
        model.optimize()
...
def createAndSolveSimpleModel():
    ampl = makeAmplModel()
    model = solveModel(ampl)
    
    print(f"Status is {model.get_status().name}")
    assert model.get_status() == amplpy_gurobi.Status.OPTIMAL
...
def createAndSolveInfeasibleModel(presolveLevel : bool = 10):
    ...
    model = solveModel(ampl, {"pre:scale": 3, "outlev" : 1, "iisfind" : 1})
    print(f"Status is {model.get_status().name}")
    assert model.get_status() == amplpy_gurobi.Status.INFEASIBLE
