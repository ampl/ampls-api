...
highs driver for AMPL.  This package uses AMPLS-API, which is an open source set of lightweight
...
def link_args():
    rpaths = [
        os.path.join('amplpy_highs', 'libs', 'ampls', libdir()),
        os.path.join('amplpy_highs', 'libs', 'highs', 'lib', libdir()),
    ]
    if OSTYPE == 'Darwin':
...
setup(
    ...
    ext_modules=[Extension(
        ...
        include_dirs=[
            os.path.join('amplpy_highs', 'libs', 'highs', 'include'),
            os.path.join('amplpy_highs', 'swig'),
            os.path.join('amplpy_highs', 'cpp', 'highs', 'include'),
            os.path.join('amplpy_highs', 'cpp', 'ampls', 'include'),
        ],
        libraries=['highs', 'highs-lib'],
        extra_compile_args=compile_args(),
        extra_link_args=link_args(),
        sources=[
            os.path.join('amplpy_highs', 'swig',
                         'amplpy_highs_swig_wrap.cxx')
        ] + [
            os.path.join('amplpy_highs', 'cpp', 'ampls', 'src', fname)
            for fname in ls_dir(os.path.join('amplpy_highs', 'cpp', 'ampls', 'src'))
            if fname.endswith(('.c', '.cpp'))
        ] + [
            os.path.join('amplpy_highs', 'cpp', 'highs', 'src', fname)
            for fname in ls_dir(os.path.join('amplpy_highs', 'cpp', 'highs', 'src'))
            if fname.endswith(('.c', '.cpp'))
        ],
    )],
    package_data={'': ls_dir('amplpy_highs/')},
)
