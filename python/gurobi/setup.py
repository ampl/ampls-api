...
setup(
    ...
    ext_modules=[
        Extension(
            ...
            libraries=["gurobi110", "gurobi-lib"],
            extra_compile_args=compile_args(),
            extra_link_args=link_args(),
            sources=[
                os.path.join("amplpy_gurobi", "swig", "amplpy_gurobi_swig_wrap.cxx")
            ]
            + [
                os.path.join("amplpy_gurobi", "cpp", "ampls", "src", fname)
                for fname in ls_dir(
                    os.path.join("amplpy_gurobi", "cpp", "ampls", "src")
                )
            ...
            + [
                ...
                for fname in ls_dir(
                    os.path.join("amplpy_gurobi", "cpp", "gurobi", "src")
                )
                if fname.endswith((".c", ".cpp"))
            ],
        )
