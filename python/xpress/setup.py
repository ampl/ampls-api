...
def link_args():
    rpaths = [
        os.path.join('amplpy_xpress', 'libs', 'ampls', libdir()),
        os.path.join('amplpy_xpress', 'libs', 'xpress', 'lib', libdir()),
    ]
    if OSTYPE == 'Darwin':
...
setup(
    ...
    ext_modules=[Extension(
        ...
        include_dirs=[
            os.path.join('amplpy_xpress', 'libs', 'xpress', 'include'),
            os.path.join('amplpy_xpress', 'swig'),
            os.path.join('amplpy_xpress', 'cpp', 'xpress', 'include'),
            os.path.join('amplpy_xpress', 'cpp', 'ampls', 'include'),
        ],
        libraries=['xprs', 'xpress-lib'],
        extra_compile_args=compile_args(),
        extra_link_args=link_args(),
        sources=[
            os.path.join('amplpy_xpress', 'swig',
                         'amplpy_xpress_swig_wrap.cxx')
        ] + [
            os.path.join('amplpy_xpress', 'cpp', 'ampls', 'src', fname)
            for fname in ls_dir(os.path.join('amplpy_xpress', 'cpp', 'ampls', 'src'))
            if fname.endswith(('.c', '.cpp'))
        ] + [
            os.path.join('amplpy_xpress', 'cpp', 'xpress', 'src', fname)
            for fname in ls_dir(os.path.join('amplpy_xpress', 'cpp', 'xpress', 'src'))
            if fname.endswith(('.c', '.cpp'))
        ],
    )],
    package_data={'': ls_dir('amplpy_xpress/')},
)
