...
scip driver for AMPL. This package uses AMPLS-API, which is an open source set of lightweight
...
def link_args():
    rpaths = [
        # os.path.join('amplpy_scip', 'libs', 'ampls', libdir()),
...
setup(
    name="amplpy_scip",
    version="0.1.4",
    description="scip extension for amplpy",
    ...
    ext_modules=[
        Extension(
            ...
            include_dirs=[
                os.path.join("amplpy_scip", "libs", "scip", "include"),
                os.path.join("amplpy_scip", "swig"),
                os.path.join("amplpy_scip", "cpp", "scip", "include"),
                os.path.join("amplpy_scip", "cpp", "ampls", "include"),
            ],
            libraries=["libscip", "scip-lib"],
            extra_compile_args=compile_args(),
            extra_link_args=link_args(),
            sources=[
                os.path.join("amplpy_scip", "swig", "amplpy_scip_swig_wrap.cxx")
            ]
            + [
                os.path.join("amplpy_scip", "cpp", "ampls", "src", fname)
                for fname in ls_dir(
                    os.path.join("amplpy_scip", "cpp", "ampls", "src")
                )
                if fname.endswith((".c", ".cpp"))
            ]
            + [
                os.path.join("amplpy_scip", "cpp", "scip", "src", fname)
                for fname in ls_dir(
                    os.path.join("amplpy_scip", "cpp", "scip", "src")
                )
                if fname.endswith((".c", ".cpp"))
            ],
        )
    ],
    package_data={"": ls_dir("amplpy_scip/")},
)
